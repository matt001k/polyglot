...
bool isSpecified(int argc, const char * argv[], const std::string &parmName, std::string &fName)
{
    for (int i = 0; i < argc; i++)
    {
        if (std::string(argv[i]) == parmName)
        {
            ...
            throw CUsageErr(std::string("\nERROR: File name is missing after '") + parmName + "' command line option");
        }
    }
...
void redirStdinToCfgFile( int argc, const char * argv[])
{
    ...
    if( fileNameCnt == 1)
    {
        ...
        {
            char msg[ 128];
            sprintf( msg, /*SIZEOF_ARRAY( msg),*/ "Error %d opening configuration file", errno);
            throw CCustErr( msg);
        }
    }
    else
    {
        throw CUsageErr( "\nERROR: Command line must specify 1 configuration file");
    }
}
...
int main( int argc, const char * argv[])
{
    ...
    {
        std::cerr << e.msg() << "\n\n";
        //printCmdLineHelp();    // Disabled since it's hard to see error message with all that chaff.
    }
        ...
        if( std::string( argv[ i]) == "--serial-nums")
        {
            ...
            if( i < argc)
            {
                if( std::string( argv[ i]) == "{")
                {
                    ...
                    throw CUsageErr( "\nERROR: Invalid serial number command line option");
                }
                else if( std::string( argv[ i]) == "GUID")
                {
                    // User requests SN auto-generation
                    if( mayAutoGen)
                    {
                        ASSERT( m_SN.empty());
...
void CSerNumSet::assertUniquness() const
{
    ...
    for( size_t i = 0; i < m_SN.size() - 1; i++)
    {
        for( size_t j = i + 1; j < m_SN.size(); j++)
        {
            if( m_SN[ i] == m_SN[ j])
            {
                throw CUsageErr( "\nERRROR: Identical serial numbers");
            }
        }
    }
}
...
DWORD decimalParm( int argc, const char * argv[], const std::string &parmName)
{
    ...
    throw CUsageErr( std::string( "\nERROR: Invalid or missing ") + parmName + " command line option");
}
