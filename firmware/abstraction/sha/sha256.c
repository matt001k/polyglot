...
#define SHA_CB_EXPAND(start, update, finish) { start, update, finish }
...
BL_STATIC SHA_t sha = { false, SHA_CFG(SHA_CB_EXPAND) };
...
BL_Err_t SHA256_Calculate(BL_UINT8_T *data, BL_UINT32_T size)
{
  BL_Err_t ret = BL_EIO;
  if(!sha.started) {
    sha.cb.start();
