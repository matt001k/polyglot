...
#define READ_WRITER_OP(node, data, size)                                       \
  RUN_WRITER_OP(NVM_Read(node, data, size))
...
void test_WriterWrite(void)
{
  ...
  TEST_ASSERT(Writer_WriteData(writer.write.buf, writer.write.size) != BL_OK);
  TEST_ASSERT(Writer_Finish() == BL_OK);
}
...
void test_WriterWriteEraseRead(void)
{
  ...
  TEST_ASSERT(Writer_Finish() == BL_OK);
  READ_WRITER_OP(TEST_PARTITION, writer.read.buf, &writer.read.size);
  TEST_ASSERT(memcmp(writer.read.buf, writer.write.buf, writer.read.size) == 0);
