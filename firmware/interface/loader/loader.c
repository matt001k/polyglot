...
{
  BL_STATIC states_e state = LOADER_INIT;
...
BL_STATIC BL_Err_t loader_Table(void)
{
  ...
  err              = NVM_Read(loader.node, (BL_UINT8_T *)&loader.table, &size);
  if(err == BL_OK && size) {
    err = Table_WritePartition(PARTITION_CURRENT, &loader.table);
    if(err == BL_OK) {
...
BL_STATIC BL_Err_t loader_Erase(void)
{
  ...
  err = NVM_Erase(APPLICATION_NODE, size);
  if(err == BL_OK) {
...
BL_STATIC BL_Err_t loader_Read(void)
{
  ...
  err          = NVM_Read(loader.node, data, &loader.size.buf);
  return err;
}
...
{
  BL_Err_t err = BL_ERR;
  err          = AES_Decrypt(loader.p.in, loader.p.out, loader.size.buf);
  return err;
}
